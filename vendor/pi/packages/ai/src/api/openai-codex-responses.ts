...
function compressRequestBodyZstd(bodyJson: string): Uint8Array | null {
	...
	if (!zlib || typeof (zlib as any).zstdCompressSync !== "function") {
		return null;
	}
	try {
		const compressed = (zlib as any).zstdCompressSync(bodyJson, {
			params: { [(zlib as any).constants.ZSTD_c_compressionLevel]: REQUEST_COMPRESSION_ZSTD_LEVEL },
		});
