...
async function runLoop(initialContext, newMessages, initialConfig, signal, emit, streamFunction) {
    ...
    while (true) {
        ...
        console.log(JSON.stringify({ level: 'info', vendor: 'pi', sessionId: config.sessionId, requestId: config.requestId, event: 'outer_loop_iteration', pending: pendingMessages.length }));
        // Inner loop: process tool calls and steering messages
        while (hasMoreToolCalls || pendingMessages.length > 0) {
            console.log(JSON.stringify({ level: 'info', vendor: 'pi', sessionId: config.sessionId, requestId: config.requestId, event: 'inner_loop_iteration', hasMoreToolCalls, pending: pendingMessages.length }));
            if (!firstTurn) {
                await emit({ type: "turn_start" });
            }
            else {
            ...
            console.log(JSON.stringify({ level: 'info', vendor: 'pi', sessionId: config.sessionId, requestId: config.requestId, event: 'stream_assistant_response_start' }));
            const message = await streamAssistantResponse(currentContext, config, signal, emit, streamFunction);
            console.log(JSON.stringify({ level: 'info', vendor: 'pi', sessionId: config.sessionId, requestId: config.requestId, event: 'stream_assistant_response_end', stopReason: message.stopReason, toolCalls: message.content.filter((c) => c.type === 'toolCall').length }));
            newMessages.push(message);
            ...
            if (toolCalls.length > 0) {
                ...
                console.log(JSON.stringify({ level: 'info', vendor: 'pi', sessionId: config.sessionId, requestId: config.requestId, event: 'execute_tool_calls_start', count: toolCalls.length }));
                const executedToolBatch = message.stopReason === "length"
                    ? await failToolCallsFromTruncatedMessage(toolCalls, emit)
                    : await executeToolCalls(currentContext, message, config, signal, emit);
                console.log(JSON.stringify({ level: 'info', vendor: 'pi', sessionId: config.sessionId, requestId: config.requestId, event: 'execute_tool_calls_end', messages: executedToolBatch.messages.length, terminate: executedToolBatch.terminate }));
