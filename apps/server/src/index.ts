...
import { registerAgentVersionRoutes } from './routes/agent-versions.js';
...
import { NotificationService } from '@workforge/monitoring';
...
import { registerSolutionRoutes } from './routes/solutions.js';
...
import { AgentService, AgentVersionService } from '@workforge/agents';
...
// Extend Fastify types to include requestId, userId, tenantId, userRole
...
export async function createServer(options: ServerOptions = {}): Promise<ServerResult> {
  ...
  // Inject requestId into every request for end-to-end tracing.
  // Tenant ID is NOT set here — it is resolved later from JWT (trusted) or
  // API Key binding during authentication. The x-tenant-id header is only
  // accepted from trusted internal proxies as a fallback.
  ...
  if (!options.testMode) {
    server.addHook('onRequest', async (request, reply) => {
      ...
      //   1. JWT claim (trusted)
      //   2. API Key binding
      //   3. x-tenant-id (only from trusted internal proxy)
      //   4. 'default'
      ...
      // Consistency check: if x-tenant-id header is present and differs from JWT claim, reject
      const headerTenantId = request.headers['x-tenant-id'] as string | undefined;
      if (headerTenantId && jwtTenantId && headerTenantId !== jwtTenantId) {
        // Log audit for tenant mismatch
        logger.warn('Tenant ID mismatch detected', { jwtTenantId, headerTenantId, ip: request.ip });
        return reply.code(403).send({ error: 'Forbidden: tenant ID mismatch' });
      }

      // If no JWT tenant, use x-tenant-id only if from trusted proxy
      if (!jwtTenantId && headerTenantId) {
        const isTrustedProxy = request.headers['x-forwarded-by'] === process.env.INTERNAL_PROXY_SECRET;
        if (isTrustedProxy) {
      ...
      // In production, fetch plan from DB; here we use a simple default
      const plan = 'free';
      const maxRequests = planLimits[plan] || planLimits.free;
      request.log = request.log.child({ tenantId, plan, maxRequests });
    });
  }
  ...
  if (options.testMode && options.services) {
    ...
    const buildProviders = (): ProviderConfig[] => {
      ...
      const list: ProviderConfig[] = [
        ...
        // Chinese model providers - using custom provider config with explicit baseUrl/models
        { id: 'qwen', apiKey: liveKey('qwen'), getApiKey: liveKey, baseUrl: 'https://dashscope.aliyuncs.com/compatible-mode/v1', models: [
          { id: 'qwen-turbo', name: 'Qwen Turbo' },
          { id: 'qwen-plus', name: 'Qwen Plus' },
          { id: 'qwen-max', name: 'Qwen Max' },
        ] as any[]},
        { id: 'zhipu', apiKey: liveKey('zhipu'), getApiKey: liveKey, baseUrl: 'https://open.bigmodel.cn/api/paas/v4', models: [
          { id: 'glm-4-flash', name: 'GLM-4 Flash' },
          { id: 'glm-4', name: 'GLM-4' },
          { id: 'glm-4-plus', name: 'GLM-4 Plus' },
        ] as any[]},
        { id: 'wenxin', apiKey: liveKey('wenxin'), getApiKey: liveKey, baseUrl: 'https://qianfan.baidubce.com/v2', models: [
          { id: 'ernie-speed-128k', name: 'ERNIE Speed' },
          { id: 'ernie-4.0-8k', name: 'ERNIE 4.0' },
        ] as any[]},
        // Local models (Ollama) - no API key needed
        { id: 'ollama', apiKey: 'ollama', baseUrl: process.env.OLLAMA_BASE_URL || 'http://localhost:11434/v1', models: [
  ...
  const deps = {
    ...
    get agentVersionService() { return agentVersionService; },
    get debugManager() { return debugManager; },
    get notificationService() { return notificationService; },
    get auditService() { return auditService; },
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- TODO(lint-any): 历史动态边界, 类型待收紧
    ...
    notificationService = new NotificationService(database, server.log);

    // Audit service.
    const { AuditService, BillingService, startBillingScheduler } = await import('@workforge/governance');
    auditService = new AuditService();
    auditService.setDatabase(database);

    // Billing scheduler
    const billing = new BillingService();
    billing.setDatabase(database);
    startBillingScheduler(billing);
    server.log.info('Billing scheduler started');

    // Agent management service (CRUD + AI generation).
    agentService = new AgentService(database, server.log as any, modelRuntime!);
    const agentVersionService = new AgentVersionService(database, server.log);
    ...
    // Seed industry templates (idempotent — only inserts if not exists)
    try {
      const { INDUSTRY_TEMPLATES } = await import('@workforge/templates');
      const { TemplateService } = await import('@workforge/templates');
      const tmplSvc = new TemplateService(database);
      for (const tmpl of INDUSTRY_TEMPLATES) {
        const existing = await tmplSvc.getById(tmpl.id);
        if (!existing) {
          await tmplSvc.create('system', null, {
            name: tmpl.name,
      ...
      server.log.info({ count: INDUSTRY_TEMPLATES.length }, 'Industry templates seeded');
    } catch (error) {
      server.log.warn({ error }, 'Industry template seeding failed (non-critical)');
    }
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- TODO(lint-any): 历史动态边界, 类型待收紧
    ...
    // Domain routers (see ./routes/*) — registered AFTER database init so deps.database is available.
    registerKnowledgeRoutes(server, { knowledgeService });
    registerNotificationRoutes(server, deps);
    registerAgentVersionRoutes(server, deps);
    ...
    registerTemplateRoutes(server, deps);
    registerScheduleRoutes(server, deps);
    registerGovernanceRoutes(server, deps);
    registerRbacRoutes(server, deps);
    registerAuditRoutes(server, deps);
    registerApprovalRoutes(server, deps);
    registerBillingRoutes(server, deps);
    ...
    registerPluginRoutes(server, deps);
    registerMcpRoutes(server, deps);
    registerLicenseRoutes(server, deps);
    registerObservabilityRoutes(server, deps);
    registerGatewayRoutes(server, deps);
    registerEvalRoutes(server, deps);
    registerSolutionRoutes(server, deps);
    registerCloudRoutes(server, deps);
    registerSsoRoutes(server, deps);
  }
