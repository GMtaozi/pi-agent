...
export class CostAnalyzer {
  ...
  private async groupBy(column: 'model' | 'agent_id', filter: CostFilter, limit: number): Promise<CostBreakdownRow[]> {
    if (!this.db) return [];
    const { clause, params } = this.buildWhere(filter);
    const keyExpr = column === 'agent_id' ? "COALESCE(agent_id, 'unassigned')" : 'model';
