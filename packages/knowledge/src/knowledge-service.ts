...
export class KnowledgeBaseService {
  ...
  async search(kbId: string, query: string, options: SearchOptions = {}): Promise<SearchResult[]> {
    ...
    `;
    const params: any[] = [JSON.stringify(queryEmbedding), kbId];

    if (options.filterByDocument && options.filterByDocument.length > 0) {
      sql += ` AND dc.document_id = ANY($3)`;
      params.push(options.filterByDocument);
    }

    sql += ` AND 1 - (dc.embedding <=> $1) >= ${scoreThreshold}`;
    sql += ` ORDER BY score DESC LIMIT ${topK}`;

    const result = await this.db.query('document_chunks', sql, params);
  ...
  private async keywordSearch(kbId: string, query: string, limit: number): Promise<SearchResult[]> {
    ...
    const likeConditions = terms.map(() => `dc.content ILIKE ?`).join(' OR ');
